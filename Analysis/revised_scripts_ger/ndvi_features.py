...
# 3. Load yearly target-crop mask for Germany
...
#
# Crop mask:
# annual ImageCollection uploaded by you for Germany
# expected values:
# - 1 = target crop (wheat)
# - 0 = not target crop
# ------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    task = ee.batch.Export.table.toDrive(
        ...
        description=f"ndvi_features_{CONFIG['export_suffix']}",
        folder=CONFIG["export_folder"],
        fileNamePrefix=f"ndvi_features_{CONFIG['export_suffix']}",
        fileFormat="CSV",
        selectors=selectors
    )
