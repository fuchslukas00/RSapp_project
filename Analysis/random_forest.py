...
DATA_CSV = PROJECT_DIR / "Analysis" / "merged_csv" / "model_table_ww_2017_2023.csv"

OUTPUT_DIR = PROJECT_DIR / "Analysis" / "model_results"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

METRICS_CSV = OUTPUT_DIR / "rf_leave_one_year_out_metrics.csv"
IMPORTANCE_RF_CSV = OUTPUT_DIR / "rf_feature_importance_mean.csv"
IMPORTANCE_PERM_CSV = OUTPUT_DIR / "rf_permutation_importance_mean.csv"
PREDICTIONS_CSV = OUTPUT_DIR / "rf_leave_one_year_out_predictions.csv"
...
# Basic checks
required_cols = {"nuts_id", "year", "yield_value"}
...
df = df.dropna(subset=["yield_value"]).copy()
...
exclude_cols = ["nuts_id", "year", "yield_value"]
...
for test_year in years:
    ...
    y_train = df.loc[train_mask, "yield_value"]

    X_test = df.loc[test_mask, feature_cols]
    y_test = df.loc[test_mask, "yield_value"]
    ...
    fold_predictions = pd.DataFrame({
        "nuts_id": df.loc[test_mask, "nuts_id"].values,
